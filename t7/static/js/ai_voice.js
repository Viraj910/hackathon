...
class AIHospitalAssistant {
    ...
    extractAndFillData(input) {
        ...
        const nameMatch = this.extractName(correctedInput);
        if (nameMatch) {
        ...
        // Extract age
        const ageMatch = correctedInput.match(/\b(\d{1,3})\s*(?:years?\s*old|age)\b/i) || 
                        correctedInput.match(/\bage\s*(?:is\s*)?(\d{1,3})\b/i) ||
                        correctedInput.match(/\bi\s*am\s*(\d{1,3})\b/i) ||
                        correctedInput.match(/(\d{1,3})\s*years?\s*old/i);
        if (ageMatch) {
        ...
        const phoneMatch = correctedInput.match(/\b\d{3}[-.\s]?\d{3}[-.\s]?\d{4}\b/) ||
                          correctedInput.match(/\b\d{10}\b/) ||
                          correctedInput.match(/\(\d{3}\)\s*\d{3}[-.\s]?\d{4}/);
        if (phoneMatch) {
        ...
        // Extract email
        const emailMatch = correctedInput.match(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/);
        if (emailMatch) {
